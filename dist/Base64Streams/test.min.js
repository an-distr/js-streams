import{Base64Decoder as r,Base64Encoder as s}from"./Base64Streams.min.js";import{Utf8DecoderStream as d,Utf8EncoderStream as c}from"../Utf8Streams/Utf8Streams.min.js";const h=t=>new ReadableStream({start(e){for(const a of t)e.enqueue(a);e.close()}}),o=t=>new TransformStream({transform(e,a){console.log(t,JSON.stringify(e)),a.enqueue(e)}}),n={encoded:"",decoded:""},l=t=>new TransformStream({transform(e,a){t.encoded+=e,a.enqueue(e)}}),p=t=>new WritableStream({write(e){t.decoded+=e}}),i=[`Unicode, formally The Unicode Standard,[note 1] is a text encoding standard maintained by the Unicode Consortium designed to support the use of text written in all of the world's major writing systems. Version 15.1 of the standard[A] defines 149813 characters[3] and 161 scripts used in various ordinary, literary, academic, and technical contexts.
  ...
  Thus far, the following versions of The Unicode Standard have been published. Update versions, which do not include any changes to character repertoire, are signified by the third number (e.g., "version 4.0.1") and are omitted in the table below.[21]`];console.groupCollapsed("Testing"),await h(i).pipeThrough(o("Source :")).pipeThrough(new c).pipeThrough(new s().transformable()).pipeThrough(o("Encoded:")).pipeThrough(l(n)).pipeThrough(new r().transformable()).pipeThrough(new d).pipeThrough(o("Decoded:")).pipeTo(p(n)),console.log("Base64:",n.encoded,"(",n.encoded.length,")"),console.log("Result:",n.decoded,"(",n.decoded.length,")"),console.groupEnd(),console.assert(n.decoded===i.join(""),"data=[",JSON.stringify(i.join("")),"](",i.join("").length,")","result=[",JSON.stringify(n.decoded),"](",n.decoded.length,"),"),console.log("Test completed.");
