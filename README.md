...
* For the first time, execute ```node_prepare.sh``` or ```node_prepare.bat```.
  ...
  **If you are using [Proto](https://moonrepo.dev/proto), just run ```pnpm install```.**
* Run tasks.

  |Task|Node|Deno|Bun|
  |-|-|-|-|
  |Build|```(pnpm or npm) run build```|```deno task build```|```bun run build-bun```|
  |Test for browser|```(pnpm or npm) run test-web```|```deno task test-web```|```bun run test-web```|
  |Test for Node|```(pnpm or npm) run test-node```|```deno task test-node```|```bun run test-node```|
  |Test for Deno|```(pnpm or npm) run test-deno```|```deno task test-deno```|```bun run test-deno```|
  |Test for Bun|```(pnpm or npm) run test-bun```|```deno task test-bun```|```bun run test-bun```|
